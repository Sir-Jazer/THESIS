<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-3xl text-gray-100 dark:text-gray-100 leading-tight">
            Create Section - {{ $program->code }}
        </h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.programs.sections.store', $program) }}" class="bg-slate-800 dark:bg-slate-800 p-6 shadow-lg sm:rounded-lg space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select id="year_level" name="year_level" class="mt-1 block w-full text-base border-slate-600 rounded-md dark:bg-slate-700 dark:text-gray-100 dark:border-slate-600" required>
                        ...
                        @for ($year = 1; $year <= 6; $year++)
                            <option value="{{ $year }}" @selected((int) old('year_level') === $year)>Year {{ $year }}</option>
                        @endfor
                    </select>
                </div>
                ...
                <div>
                    ...
                    <x-text-input id="section_code" name="section_code" class="mt-1 block w-full text-base" :value="old('section_code')" required />
                </div>
                ...
                <div class="flex justify-between">
                    ...
                    <button class="px-4 py-2.5 text-base bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition duration-150 ease-in-out">Save Section</button>
                </div>
            </form>
        </div>
    </div>
